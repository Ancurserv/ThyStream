...
3. What if someone just modifies the application and manually replace my Session ID with theirs?

Oh this is the funny part, anyone that attempts to do such thing will just regret doing it, because if they were to replace their session ID with yours then nothing would happen, literally nothing would happen. Because the Session ID is unique and already exists, the other Session ID replacement will simply not work. The only way a person could perform any hack what so ever is if they directly had access to your device. All remote hacking attempts will just fail.
